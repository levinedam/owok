        ...
        while True:
            ...
            if not self.bot.paused and cfg.get('enabled', False):
                ...
                if now - self.last_sell_time > cooldown:
                    ...
                    await self.bot.send_message(f"sell {sell_type}", priority=True, skip_typing=True)
                    ...
                    if current_cash is None:
                        await self.bot.send_message(f"{self.bot.prefix}cash", priority=True, skip_typing=True)
                    else:
    ...
    async def on_message(self, message):
        ...
        if "don't have enough cowoncy" in content or "not enough cowoncy" in content:
            if time.time() - self.last_sell_time > 300:
                ...
                await self.bot.send_message(f"sell {cfg.get('type', 'all')}", priority=True, skip_typing=True)
                self.last_sell_time = time.time()
