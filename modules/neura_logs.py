...
class NeuraLogs:
        ...
        try:
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    ...
                    # Sync with state for dashboard entries
    ...
    def log(self, bot, log_type, message):
        ...
        # Override with logmisc config if available
        for k, v in type_colors.items():
            colors[k] = v.replace('#', '') # Rich prefers names or simple hex without # sometimes
        ...
        if log_type == "STEALTH":
            ...
            # Handle hex colors (Rich supports [#ffffff])
            if log_type in type_colors:
                rich_color = type_colors[log_type]
                self.console.print(f"{name_tag}[dim]{t}[/dim] [[bold {rich_color}]{log_type}[/bold {rich_color}]]  {message}")
            else:
                self.console.print(f"{name_tag}[dim]{t}[/dim] [[bold {color}]{log_type}[/bold {color}]]  {message}")

        # Integrate with state command tracking
        import core.state as state
