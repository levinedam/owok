...
def check_rate_limit(ip):
    ...
    if ip in LOGIN_ATTEMPTS:
        ...
        if now - block_time > BLOCK_DURATION: # Reset if block expired
...
def stats():
    ...
    # If stats don't exist yet, return empty (bot not ready)
    if not st:
        return jsonify({})
...
def accounts_api():
    if request.method == 'POST':
        ...
        try:
            ...
            # Re-sync accounts? Complex logic. For now just save.
            # Ideally we would restart bots or update their instance.
            # Simplified: Update attribute.
            for bot in state.bot_instances:
